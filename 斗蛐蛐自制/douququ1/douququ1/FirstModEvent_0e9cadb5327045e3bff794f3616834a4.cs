...
{
	public class FirstModEvent_0e9cadb5327045e3bff794f3616834a4 : TaiwuEventItem
	{
		public FirstModEvent_0e9cadb5327045e3bff794f3616834a4()
		{
			//事件GUID
			Guid = Guid.Parse("0e9cadb5-3270-45e3-bff7-94f3616834a4");
			//该事件是否是头部事件
			IsHeadEvent = true;
			//事件分组
		...
		{
			int npcID = 0;
			ArgBox.Get("CharacterId", ref npcID);
			ArgBox.Set("npcID", npcID);
		  	var npc = EventHelper.GetCharacterById(npcID);
		   	int selectNextMonth = 0;
		    TaiwuEvent.GetModData("SelectNextMonth",true,ref selectNextMonth);
		    if (selectNextMonth == 1)
		    {
			    return false;
		    }
			...
			{
				if (itemKey.TemplateId == 11061)
				{
					flag = true;
				}
			}
			var npcOrganizationInfo = npc.GetOrganizationInfo();
			if (npcOrganizationInfo.OrgTemplateId.ToString() == "16" && flag)
			{
				return true;
			}
			else
